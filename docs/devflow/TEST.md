...
| `test_project/tests/core/test_eq_relation_graph.gd` + resolution/snapshot relation tests | Relation / Determinism | sorted actor adjacency、query copy、BFS order、TREE/GRAPH、serial suture、duplicate-id restore reindex、trace/snapshot parity | EQM-139 |
| `test_project/tests/core/test_eq_event_lines.gd` + snapshot/pipeline tests | Event-line / Determinism | watched-key content order、membership-only input、effective-rate cache mutation/restore、zero/negative poll、trace/snapshot parity | EQM-140 |
| `test_project/tests/trigger/test_eq_reaction_fire_gate.gd` | Trigger / Condition / Snapshot | pure preview、named WAIT→FIRE、invalidation-wins、schedule-before-commit atomicity、multi-view/duplicate slot、arm-time relative/COUNTER bind、lifecycle cleanup、schema-v8 exact/tamper continuation、post-arm definition edit、historical fail-closed | EQM-141 |
| `test_project/tests/trigger/test_eq_trigger_slot_snapshot.gd` | Trigger / Snapshot | duplicate arm slotごとのrumination/FIRE index、arm-time duration/use固定、finite-expiry save/load continuation、gate/watch cleanup | EQM-141 repair review |
...
| `test_project/tests/performance/` | Scheduler / Trigger / State / Relation / Event-line | EQM-local deterministic work-count gates and environment-labelled elapsed observations; trigger, relation, watched-line selection, and effective-rate A/B are performance-only | EQM-102, EQM-112, EQM-134, EQM-136, EQM-138, EQM-139, EQM-140 |
