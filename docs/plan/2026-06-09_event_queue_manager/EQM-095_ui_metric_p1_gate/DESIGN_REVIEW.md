...
| 項目 | 記録 |
|---|---|
| 対象 | `docs/plan/2026-06-09_event_queue_manager/EQM-095_ui_metric_p1_gate/` |
| 主分類 | editor UI / UX |
| 判断 | `needs_design_update` |
| queue status | `COMPLETE` |
| 採用判断 | UI metric P1 gate with calibrated thresholds (adoption M5). |
| 棄却 / 延期判断 | 旧UX/暫定contractは設計内の reject/defer/self-review に従う。追加の互換維持要求は見つからない。 |
| 実装固定点 | Row geometry, truncation, and picker width thresholds updated from calibration ledger evidence; P1 gate active; exceptions declared in the contract. |
...
| product proof gate | Row geometry, truncation, and picker width thresholds updated from calibration ledger evidence; P1 gate active; exceptions declared in the contract. |
...
- self-review: docs/review/autopilot/EQM-095_SELF_REVIEW_2026-06-18.md; target files present: docs/ui/EDITOR_UI_CONTRACT.md, tests/ui_headless/; target files missing: none detected by path scan.
