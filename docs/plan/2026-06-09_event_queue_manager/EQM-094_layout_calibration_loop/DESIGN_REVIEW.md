...
| 項目 | 記録 |
|---|---|
| 対象 | `docs/plan/2026-06-09_event_queue_manager/EQM-094_layout_calibration_loop/` |
| 主分類 | editor UI / UX |
| 判断 | `needs_design_update` |
| queue status | `COMPLETE` |
| 採用判断 | Layout Calibration Loop MVP (tweak-and-bake debug tab + ledger). |
| 棄却 / 延期判断 | 旧UX/暫定contractは設計内の reject/defer/self-review に従う。追加の互換維持要求は見つからない。 |
| 実装固定点 | Debug-only tab edits layout params per ui_metric_id; Copy Layout Feedback emits schema-valid JSON per UI_LAYOUT_CALIBRATION_POLICY.md; tab hidden without flag (P0 test); bake procedure and cold-control ledger documented. |
...
| product proof gate | Debug-only tab edits layout params per ui_metric_id; Copy Layout Feedback emits schema-valid JSON per UI_LAYOUT_CALIBRATION_POLICY.md; tab hidden without flag (P0 test); bake procedure and cold-control ledger documented. |
...
- self-review: docs/review/autopilot/EQM-094_SELF_REVIEW_2026-06-18.md; target files present: addons/event_queue_manager/editor/testing/eq_calibration_tab.gd, docs/ui/LAYOUT_CALIBRATION_LEDGER.md; target files missing: none detected by path scan.
