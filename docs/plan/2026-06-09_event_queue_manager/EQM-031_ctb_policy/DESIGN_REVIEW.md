...
| 項目 | 記録 |
|---|---|
| 対象 | `docs/plan/2026-06-09_event_queue_manager/EQM-031_ctb_policy/` |
| 主分類 | policy / progression |
| 判断 | `pass` |
| queue status | `COMPLETE` |
| 採用判断 | CTB policy with speed and action cost. |
| 棄却 / 延期判断 | 旧UX/暫定contractは設計内の reject/defer/self-review に従う。追加の互換維持要求は見つからない。 |
| 実装固定点 | Tests cover faster actor extra turns, heavy action delay, wait action shorter delay, haste/slow next-turn behavior. |
...
| product proof gate | Tests cover faster actor extra turns, heavy action delay, wait action shorter delay, haste/slow next-turn behavior. |
...
- self-review: docs/review/autopilot/EQM-031_SELF_REVIEW_2026-06-15.md; target files present: resources/policies/eq_ctb_policy.gd, tests/policy/; target files missing: none detected by path scan.
