...
Current: **none**。user-approved autonomous scheduler hot-path round (2026-07-19) COMPLETE:
速度改善調査で `peek_next()` の backend 全複製が binary heap drain を支配する証拠が揃ったため、
Phase 15 (EQM-142→145) を起票し、線形に実装・検証した。live-peek、reschedule lookup、
membership API、consumer-facing backend selection はすべて COMPLETE。consumer (Amberground) が
必要対応に気付けるよう API docs に `has_event()` 移行と `EQConfig.scheduler_backend`
opt-in 手順を残した。通常回帰と独立performance laneは分離を保ったまま両方PASS。
...
### EQM-143 — COMPLETE (2026-07-19) — scheduler reschedule O(1) lookup

```text
acceptance:
  - EQScheduler maintains a private event_id -> live-entry accelerator mirrored with the generation map
  - reschedule target lookup performs no backend ordered() scan/copy while preserving kind/actor/payload
  - push/reschedule/cancel/pop/restore keep the accelerator consistent; invalid tick/cancelled/popped ids reject without scan
  - scheduler docstring distinguishes O(1) target lookup from backend insertion cost
  - regression and independent performance lane both PASS
plan: docs/plan/2026-06-09_event_queue_manager/EQM-143_reschedule_o1/
tests:
  - ./tools/test.sh -> PASS (regression only; files=78 checks=2101 failures=0; run 20260719-042245-41246)
  - ./tools/test.sh --performance -> PASS (performance only; files=7 checks=64 failures=0; run 20260719-042251-41408)
performance (operation-local; advisory elapsed):
  - binary-heap reschedule N=256: legacy-shape 185,047 usec, current 779 usec, 237.54x; hard work 98,176 copied/sorted entries -> 0
review: docs/review/autopilot/EQM-143_SELF_REVIEW_2026-07-19.md
profile: docs/design/RUNTIME_PERFORMANCE_PROFILE.md
```
...
### EQM-145 — COMPLETE (2026-07-19) — consumer-facing scheduler backend selection

```text
acceptance:
  - EQConfig.SchedulerBackend and exported scheduler_backend add sorted-array default / binary-heap opt-in
  - unknown scheduler_backend values produce eqm.config.scheduler_backend_unknown
  - EQRuntime/EQManager apply backend choice at setup; non-empty reconfigure records eqm.runtime.scheduler_backend_reconfigure_nonempty and keeps existing scheduler/live events
  - sorted-array and binary-heap configured managers produce identical trace JSONL for the same scenario
  - scheduler backend choice is documented as config, not snapshot state; saves remain portable across backends
  - API docs (EN/JA) tell Amberground/consumers: no action for EQM-142/143, migrate membership checks to has_event(), opt into heap via EQConfig.scheduler_backend before seeding
plan: docs/plan/2026-06-09_event_queue_manager/EQM-145_backend_selection/
tests:
  - python3 tools/check_api_surface.py --update -> PASS (golden diff: SchedulerBackend, scheduler_backend, two stable errors)
  - ./tools/test.sh -> PASS (regression only; files=80 checks=2133 failures=0; run 20260719-043726-48423)
  - ./tools/test.sh --performance -> PASS (performance only; files=8 checks=68 failures=0; run 20260719-043732-48583)
performance (operation-local; advisory elapsed):
  - configured binary heap drain N=1024 elapsed=14ms, guard <500ms, trace/workload sentinel exact
api:
  - EQConfig.SchedulerBackend enum
  - EQConfig.scheduler_backend exported var
  - EQError.CONFIG_SCHEDULER_BACKEND_UNKNOWN
  - EQError.RUNTIME_SCHEDULER_BACKEND_RECONFIGURE_NONEMPTY
review: docs/review/autopilot/EQM-145_SELF_REVIEW_2026-07-19.md
profile: docs/design/RUNTIME_PERFORMANCE_PROFILE.md
```

Dependency sweep: EQM-145 COMPLETE → queueにREADY/BACKLOG taskなし。Current pointer → none。Phase 15 COMPLETE。
