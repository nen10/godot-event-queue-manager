...
| 項目 | 記録 |
|---|---|
| 対象 | `docs/plan/2026-06-09_event_queue_manager/EQM-101_demo_suite/` |
| 主分類 | docs / demos / package proof |
| 判断 | `needs_design_update` |
| queue status | `COMPLETE` |
| 採用判断 | Multi-genre demo suite. |
| 棄却 / 延期判断 | 旧UX/暫定contractは設計内の reject/defer/self-review に従う。追加の互換維持要求は見つからない。 |
| 実装固定点 | CTB, energy, wait-turn, action-resolution, phase, and stack demos load or are environment-blocked with proof; each demo emits its golden trace headless per DETERMINISM_TRACE_TEST_POLICY.md. |
...
| product proof gate | CTB, energy, wait-turn, action-resolution, phase, and stack demos load or are environment-blocked with proof; each demo emits its golden trace headless per DETERMINISM_TRACE_TEST_POLICY.md. |
...
- self-review: docs/review/autopilot/EQM-101_SELF_REVIEW_2026-06-18.md; target files present: demos/, docs/manual/policy_selection.md, tests/golden/; target files missing: none detected by path scan.
