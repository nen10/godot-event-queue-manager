...
- authored conditionsはarm時に一度だけbindする。reactionのduration/reaction_count sugarは既存の
  expiry event/remaining_ruminationsがcanonicalなのでbound termから除外する。
- gate ownershipはreservation objectではなくtrigger-engine arm slot。definition Resourceをarm後に
  編集しても、arm-time authored spec snapshot／bound term／別slotのgateは書き換えない。
...
- save schema v8は各`armed_triggers` rowへ`solve`、`inv`、`counter_lines`を必須保存する。
  ...
  全gate間unique mappingをverify-before-mutateする。
...
同一reservationのduplicate slotでも二重消費やgate上書きが起きない。runtime gate tableとwatched-line
